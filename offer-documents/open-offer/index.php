<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="http://localhost/Gretex%20Corporate%20main/js/script.js"></script>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css" />
    <script src="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js"></script>

    <title>Open Offers - Gretex Corporate Services Limited</title>

    <link rel="stylesheet"
        href="http://localhost/Gretex%20Corporate%20main/css/offer-documents/open-offer/open-offer.css">

</head>

<body>
    <?php include "../../header.php"; ?>

    <div class="main-hero">
        <div class="hero-header">
            <h1>Open Offers</h1>
            <p>Upscale your company with experts in financial guidance. Navigate the IPO journey seamlessly with our
                strategic insights and proven expertise.</p>
        </div>
        <div class="hero-footer">
            <div class="breadcrumbs">
                <a href="http://localhost/Gretex%20Corporate%20main/">Home</a> / <span>Services</span>
            </div>
        </div>
    </div>

    <div class="general-container">

        <div class="section-header">
            <h1 class="section-heading">Role of Gretex in IPO</h1>
            <p class="section-description">Gretex acts as Lead Manager to the issue & completes entire listing
                process.</p>
        </div>

        <div class="issue">
            <div class="issue-1">

                <div class="issue-content">
                    <h2 class="ipo-headline">Pre Issue</h2>
                    <ul class="ipo-description">
                        <li>Checking the Eligibility Criteria.</li>
                        <li>Due diligence of Company's Operations, Management, Business plans, etc.</li>
                        <li>Drafting & Designing Offer documents, Prospectus, Statutory Advertisements &Memorandum of
                            Association (MoA).</li>
                        <li>Ensuring compliance with BSE, NSE, RoC& SEBI along with finalization of prospectus and RoC
                            filling.</li>
                    </ul>
                </div>

                <div class="issue-image">
                    <img src="../../assets/pre-ipo.png" alt="">
                </div>
            </div>


            <div class="issue-1">
                <div class="issue-content">
                    <h2 class="ipo-headline">POST-Issue</h2>
                    <ul class="ipo-description">
                        <li>Marketing Strategies for the listing company.</li>
                        <li>Management of Escrow Accounts, Dispatch of Refunds, Finalization of Trading & Dealing of
                            Instruments.</li>
                        <li>Submission of final Offer Document with ROC / SEBI and Stock Exchanges, Completion of Issue
                            and Allotment Process, Listing of Shares on Stock Exchange, etc.</li>
                        <li>Proper Handholding after listing until migration to the main board.</li>
                    </ul>
                </div>
                <div class="issue-image">
                    <img src="../../assets/post-issue.png" alt="">
                </div>
            </div>
        </div>
    </div>



    <!-- service process -->
    <div class="general-container">
        <div class="about-usp-container">
            <div class="usp-header">
                <h2>The process of getting<br> your IPO in the market</h2>
            </div>
            <div class="usp-row">
                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Comprehensive Due Diligence</h3>
                        <p>Thoroughly evaluates acquirer's financials, intent, and compliance to ensure regulatory
                            conditions for open offer are satisfied.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Drafting and Filing Documentation</h3>
                        <p>Prepares, reviews, and submits statutory offer documents, ensuring accuracy and compliance
                            with SEBI's disclosure requirements</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Verifying Fair Offer Pricing</h3>
                        <p>Confirms that the open offer price meets SEBI's norms and protects shareholder financial
                            interests.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Managing Escrow Account Funds</h3>
                        <p>Supervises escrow account funding, ensuring full financial backing and availability of
                            acquisition consideration for shareholders.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Liaising with Regulatory Authorities</h3>
                        <p>Coordinates with SEBI, exchanges, and depositories to maintain smooth communication and
                            regulatory compliance throughout the process.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Handling Public Shareholder Communication</h3>
                        <p>Issues public announcements and advertisements on time to keep shareholders informed and
                            maintain process transparency.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Monitoring Process and Reporting</h3>
                        <p>Tracks all open offer activities, ensures timelines are met, and files compliance reports
                            with relevant authorities.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>

                <div class="usp-data">
                    <div class="usp-data-header">
                        <h3>Safeguarding Minority Shareholder Interests</h3>
                        <p>Ensures minority shareholders receive equitable treatment and fair exit opportunity during
                            acquisition and control change events.</p>
                    </div>
                    <div class="usp-icon">
                        <span class="icon">
                            <svg viewBox="0 0 38 37" width="32" height="32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_145_13)">
                                    <path
                                        d="M7.51669 17.9125C5.08572 17.9125 3.10815 16.0045 3.10815 13.6599C3.10815 11.3151 5.08572 9.40704 7.51669 9.40704C9.94735 9.40704 11.9246 11.3148 11.9246 13.6599C11.9246 16.0048 9.94735 17.9125 7.51669 17.9125ZM7.51669 10.6006C5.76147 10.6006 4.33396 11.9731 4.33396 13.6599C4.33396 15.3465 5.76147 16.719 7.51669 16.719C9.2713 16.719 10.6988 15.3465 10.6988 13.6599C10.6988 11.9731 9.2713 10.6006 7.51669 10.6006Z"
                                        fill="url(#paint0_linear_145_13)"></path>
                                    <path
                                        d="M9.29021 29.6872H1.60199C0.718545 29.6872 0 28.9919 0 28.1373V26.1852C0 22.1891 3.37187 18.9377 7.51674 18.9377C9.14686 18.9377 10.6986 19.4331 12.0028 20.3699C12.2751 20.5657 12.3332 20.9393 12.1321 21.2047C11.931 21.4696 11.5476 21.5261 11.275 21.3306C10.1829 20.5462 8.88351 20.1312 7.51674 20.1312C4.04791 20.1312 1.22581 22.8473 1.22581 26.1852V28.1373C1.22581 28.3337 1.39459 28.4936 1.60199 28.4936H9.29051C9.62869 28.4936 9.90342 28.7609 9.90342 29.0904C9.90342 29.4197 9.62869 29.6872 9.29021 29.6872Z"
                                        fill="url(#paint1_linear_145_13)"></path>
                                    <path
                                        d="M36.398 29.2355H28.7098C28.3716 29.2355 28.0969 28.9683 28.0969 28.6388C28.0969 28.3095 28.3716 28.042 28.7098 28.042H36.3983C36.6057 28.042 36.7745 27.8823 36.7745 27.6859V25.7339C36.7745 22.3954 33.9524 19.6799 30.4836 19.6799C29.0348 19.6799 27.6204 20.1656 26.5009 21.048C26.2381 21.2552 25.8524 21.2158 25.639 20.9597C25.4262 20.7035 25.4663 20.3276 25.73 20.1202C27.0668 19.0668 28.755 18.4863 30.4836 18.4863C34.6281 18.4863 38 21.7374 38 25.7339V27.6859C38 28.5403 37.2815 29.2355 36.398 29.2355Z"
                                        fill="url(#paint2_linear_145_13)"></path>
                                    <path
                                        d="M30.4833 17.6177C28.0526 17.6177 26.0753 15.7099 26.0753 13.3651C26.0753 11.0202 28.0526 9.11218 30.4833 9.11218C32.9142 9.11218 34.8918 11.0199 34.8918 13.3651C34.8918 15.7102 32.9139 17.6177 30.4833 17.6177ZM30.4833 10.3057C28.7286 10.3057 27.3011 11.6782 27.3011 13.3651C27.3011 15.052 28.7286 16.4241 30.4833 16.4241C32.2385 16.4241 33.666 15.052 33.666 13.3651C33.666 11.6782 32.2382 10.3057 30.4833 10.3057Z"
                                        fill="url(#paint3_linear_145_13)"></path>
                                    <path
                                        d="M27.3185 31.629H10.6816C9.57637 31.629 8.67737 30.7601 8.67737 29.6919V26.9463C8.67737 21.4594 13.3077 16.9955 19.0001 16.9955C24.6925 16.9955 29.3228 21.4594 29.3228 26.9463V29.6919C29.3228 30.7601 28.4237 31.629 27.3185 31.629ZM19.0001 18.1894C13.984 18.1894 9.90317 22.118 9.90317 26.9466V29.6922C9.90317 30.1021 10.2524 30.4358 10.6816 30.4358H27.3182C27.7474 30.4358 28.0966 30.1021 28.0966 29.6922V26.9466C28.0969 22.118 24.0161 18.1894 19.0001 18.1894Z"
                                        fill="url(#paint4_linear_145_13)"></path>
                                    <path
                                        d="M18.7869 16.2982C15.6637 16.2982 13.1223 13.8473 13.1223 10.8343C13.1223 7.82159 15.6634 5.37097 18.7869 5.37097C21.9107 5.37097 24.4517 7.82188 24.4517 10.8346C24.4517 13.8476 21.9104 16.2982 18.7869 16.2982ZM18.7869 6.56452C16.3395 6.56452 14.3481 8.48014 14.3481 10.8346C14.3481 13.1894 16.3395 15.105 18.7869 15.105C21.2346 15.105 23.2259 13.1894 23.2259 10.8346C23.2259 8.48014 21.2346 6.56452 18.7869 6.56452Z"
                                        fill="url(#paint5_linear_145_13)"></path>
                                </g>
                                <defs>
                                    <linearGradient id="paint0_linear_145_13" x1="10.7499" y1="11.32" x2="2.69112"
                                        y2="12.5658" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint1_linear_145_13" x1="10.6194" y1="21.3553" x2="-0.525163"
                                        y2="23.2496" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint2_linear_145_13" x1="36.3347" y1="20.9039" x2="24.9797"
                                        y2="22.8728" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint3_linear_145_13" x1="33.717" y1="11.0251" x2="25.6583"
                                        y2="12.2709" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint4_linear_145_13" x1="26.5718" y1="20.2867" x2="8.06896"
                                        y2="24.1798" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <linearGradient id="paint5_linear_145_13" x1="22.9421" y1="7.82859" x2="12.5865"
                                        y2="9.4298" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#E24C4A"></stop>
                                        <stop offset="1" stop-color="#386BB7"></stop>
                                    </linearGradient>
                                    <clipPath id="clip0_145_13">
                                        <rect width="38" height="37" fill="white"></rect>
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                    </div>
                </div>
            </div>
        </div>
    </div>


    <div class="general-container">
        <div class="section-header">
            <h2>IPO Success Stories</h2>
            <p>Undoubtedly the year 2022 was the year of IPOs. But, the year
                was a rollercoaster for any investor tracking the IPO market. Due to increased volatility brought on by
                geopolitical unrest and economic turmoil, the Initial Public Offering (IPO) market saw mixed results in
                2022, following a record-breaking performance in 2021.</p>
        </div>

        <div class="ipo-container">
            <div class="ipo-row">

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/deepti-alloy-steel-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/deepti-alloy-steel-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/deepti-alloy-steel-limited">Deepti
                                Alloy <br> Steel Limited</a>
                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/kwality-credit-and-leasing-limited-thumbnail.png" alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/kwality-credit-and-leasing-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/kwality-credit-and-leasing-limited">Kwality
                                Credit and <br> Leasing Limited</a>
                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/econo-trade-india-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/econo-trade-india-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/econo-trade-india-limited">Econo
                                Trade <br> (India) Limited</a>
                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/neil-industries-limited-min-thumbnail.png" alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/neil-industries-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/neil-industries-limited">Neil
                                Industries <br> Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/ph-trading-limited-thumbnail.png" alt="">
                            <div class="ipo-cta-button">
                                <a href="http://localhost/Gretex%20Corporate%20main/ipo/a-m-febcon-limited.php"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/ph-trading-limited">PH
                                Trading <br> Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/rcl-retails-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/rcl-retails-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/rcl-retails-limited">RCL
                                Retails <br> Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="../../assets/offer-documents/images/tirrihannah-company-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/tirrihannah-company-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/tirrihannah-company-limited">Tirrihannah
                                Company <br> Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/stratmont-industries-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/stratmont-industries-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/stratmont-industries-limited">Stratmont
                                Industries <br> Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/reliance-jute-international-mills-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/reliance-jute-international-mills-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/reliance-jute-international-mills-limited">Reliance
                                Jute (International) <br> Mills Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/anshuni-commercials-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/anshuni-commercials-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/anshuni-commercials-limited">Anshuni
                                Commercials <br> Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/sharad-fibres-&-yarn-processors-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a href="http://localhost/Gretex%20Corporate%20main/ipo/aki-india-limited.php"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/sharad-fibres-&-yarn-processors-limited">Sharad
                                Fibres
                                & Yarn <br> Processors Limited</a>

                        </div>
                    </div>
                </div>

                <div class="ipo-col">
                    <div class="ipo">
                        <div class="ipo-image">
                            <img src="http://localhost/Gretex%20Corporate%20main/assets/banners/open-offers/prozone-realty-limited-thumbnail.png"
                                alt="">
                            <div class="ipo-cta-button">
                                <a
                                    href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/prozone-realty-limited"><svg
                                        xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                                        <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-miterlimit="10" stroke-width="1.5" d="m14 16l4-4m0 0l-4-4m4 4H6" />
                                    </svg></a>
                            </div>
                        </div>
                        <div class="ipo-content">
                            <a
                                href="http://localhost/Gretex%20Corporate%20main/offer-documents/open-offer/prozone-realty-limited">Prozone
                                Realty <br> Limited</a>

                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>



    <!-- Contact CTA section -->
    <div class="footer-cta-section">
        <div class="cta-section-left">
            <div class="cta-header">
                <h4>Fuel Your Company's Growth with Strategic Financial Solutions</h4>
                <p>Looking to expand your business, raise capital, or explore new financial opportunities? We specialize
                    in tailored merchant banking solutions that empower companies to scale with confidence. Get in touch
                    with us today and let's unlock new growth possibilities together.</p>
            </div>
            <div class="cta-section-button">
                <a class="cta-button-main" href="http://localhost/Gretex%20Corporate%20main/contact.php">
                    <span class="button-text">Contact us</span>
                    <svg class="cta-arrow" xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                        viewBox="0 0 24 24">
                        <path fill="#fff" fill-rule="evenodd"
                            d="M9 6.75a.75.75 0 0 1 0-1.5h9a.75.75 0 0 1 .75.75v9a.75.75 0 0 1-1.5 0V7.81L6.53 18.53a.75.75 0 0 1-1.06-1.06L16.19 6.75z"
                            clip-rule="evenodd" />
                    </svg>
                </a>
            </div>
        </div>
        <div class="cta-section-right">

        </div>
    </div>

    <?php include '../../footer.php'; ?>

    <script>
        const counters = document.querySelectorAll('.number');
        const speed = 200; // Adjust for speed of animation

        counters.forEach(counter => {
            const updateCounter = () => {
                const target = +counter.getAttribute('data-target');
                const current = +counter.innerText.replace(/[^0-9]/g, ""); // Remove non-numeric characters

                const increment = Math.ceil(target / speed);

                if (current < target) {
                    const nextValue = Math.min(current + increment, target);
                    const format = counter.getAttribute('data-format');

                    // Update number with format
                    if (format === "+") {
                        counter.innerText = `${nextValue.toLocaleString('en-IN')}+`;
                    } else if (format === "₹Cr") {
                        counter.innerText = `₹${nextValue.toLocaleString('en-IN')} Cr`;
                    }

                    setTimeout(updateCounter, 10);
                } else {
                    // Final value to ensure correct format
                    const format = counter.getAttribute('data-format');
                    if (format === "+") {
                        counter.innerText = `${target.toLocaleString('en-IN')}+`;
                    } else if (format === "₹Cr") {
                        counter.innerText = `₹${target.toLocaleString('en-IN')} Cr`;
                    }
                }
            };

            updateCounter();
        });
    </script>


</body>

</html>